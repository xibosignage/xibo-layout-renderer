...
export interface ILayoutTransitionManager {
  ...
  prepareTransition(from: Layout, to: Layout): Promise<void>;

  /** Execute fade transition with regions playing concurrently */
  executeTransition(from: Layout, to: Layout): Promise<void>;
...
export class LayoutTransitionManager implements ILayoutTransitionManager {
   ...
   */
  async prepareTransition(from: Layout, to: Layout): Promise<void> {
   ...
   */
  async executeTransition(from: Layout, to: Layout): Promise<void> {
    const startTime = performance.now();
   ...
   */
  private async waitForLayoutReady(layout: Layout, maxWaitMs: number): Promise<void> {
    const startTime = performance.now();
