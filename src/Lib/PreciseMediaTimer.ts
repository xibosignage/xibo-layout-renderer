...
export class PreciseMediaTimer implements IPreciseMediaTimer {
    ...
    constructor(durationMs: number) {
        ...
        console.debug('??? XLR.debug >> PreciseMediaTimer: duration', { durationMs });
    ...
    start(): void {
        if (this.isCurrentlyRunning) {
            console.warn('??? XLR.debug >> PreciseMediaTimer: Already running');
            return;
        }
        ...
        this.rafId = requestAnimationFrame(() => {
            ...
            for (const callback of this.tickCallbacks) {
                try {
                    ...
                    console.error('??? XLR.debug >> Error in onTick callback:', error);
                }
            }
            ...
            if (currentRemaining <= 0) {
                ...
                for (const callback of this.completeCallbacks) {
                    try {
                        ...
                        console.error('??? XLR.debug >> Error in onComplete callback:', error);
                    }
                }
            } else {
                // Schedule next tick
