...
export class RegionMediaPipeline implements IRegionMediaPipeline {
    ...
    setCurrentMedia(media: IMedia): void {
        ...
        } else if (nextMediaIndex >= this.region.mediaObjects.length) {
            // Loop back to first
    ...
    async onCurrentMediaWillEnd(timeBeforeEnd: number): Promise<void> {
        if (!this._nextMedia) {
            console.debug('??? XLR.debug >> RegionMediaPipeline: No next media to preload');
            return;
        }
        
        console.debug('??? XLR.debug >> RegionMediaPipeline: Preload trigger reached', {
    ...
    async prepareNextMediaInBackground(): Promise<void> {
        ...
        if (this.preloadPromise) {
            console.debug('??? XLR.debug >> RegionMediaPipeline: Preload already in progress');
            return;
        }
        ...
        this.preloadPromise = (async () => {
            try {
                console.debug('??? XLR.debug >> RegionMediaPipeline: Starting background preload', {
                ...
                if (this._nextMedia) {
                    ...
                    console.debug('??? XLR.debug >> RegionMediaPipeline: Preload completed', {
                        mediaId: this._nextMedia?.id,
                    });
                }
            } catch (error) {
                if (error instanceof DOMException && error.name === 'AbortError') {
                    console.debug('??? XLR.debug >> RegionMediaPipeline: Preload cancelled');
                } else {
                    console.error('??? XLR.debug >> RegionMediaPipeline: Preload failed', error);
                }
            } finally {
    ...
    async transitionToNextMedia(): Promise<void> {
        if (!this._nextMedia) {
            console.debug('??? XLR.debug >> RegionMediaPipeline: No next media, looping current');
            // TODO: Handle loop-same-media case
            return;
        }
        
        console.debug('??? XLR.debug >> RegionMediaPipeline: Starting transition', {
        ...
        try {
            ...
            console.error('??? XLR.debug >> RegionMediaPipeline: Transition failed', error);
            throw error;
        }
    }
